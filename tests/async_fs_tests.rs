...
async fn test_has_chart_file_detects_all_extensions() {
    ...
    for ext in &["bms", "bme", "bml", "pms", "bmson"] {
        ...
        tokio::fs::write(sub.join(format!("chart.{ext}")), "")
            .await
            .unwrap();
...
async fn test_has_chart_file_ignores_non_chart_files() {
    ...
    tokio::fs::write(dir.join("readme.txt"), "hello")
        .await
        .unwrap();
    tokio::fs::write(dir.join("song.mp3"), "data")
        .await
        .unwrap();
    tokio::fs::write(dir.join("image.png"), "data")
        .await
        .unwrap();
